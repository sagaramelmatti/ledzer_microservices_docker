package com.ledzer.auth.config;
...
class ResourceServer extends ResourceServerConfigurerAdapter {
	 ...
	 * http.csrf().disable();
	 * 
	 * http.authorizeRequests().antMatchers("/oauth/token").anonymous(); // Require
	 * all GET requests to have client "read" scope
	 * http.authorizeRequests().antMatchers(HttpMethod.GET,
	 * "/**").access("#oauth2.hasScope('read')"); // Require all POST requests to
	 * have client "write" scope
